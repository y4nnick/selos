...
class Val {
    ...
    public static function getPayload($service,$param,$msg=null,$code = 400){
        if($msg == "" || $msg == null) $msg = $param." wurde nicht übergeben";

        if(@$service -> _request == null || @empty($service -> _request)){
    ...
    public static function checkForNotEmpty($service,$param,$msg = "",$code = 400){
        if($msg == "" || $msg == null) $msg = "Parameter '$param' wurde nicht übergeben";
        if(@$service -> _request[$param] == null || @empty($service -> _request[$param])){
            throw new Exception($msg);
        }else{
            return $service -> _request[$param];
        }
